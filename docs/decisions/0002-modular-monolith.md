...
## Context
DevForge has a wide scope (Auth, Users, Projects, Posts, Build Logs, Reactions, Notifications). We need an architectural pattern that supports this complexity without overwhelming a single developer.

## Decision
We will use a **Modular Monolith** pattern housed within an **npm workspaces + Turborepo** monorepo.
